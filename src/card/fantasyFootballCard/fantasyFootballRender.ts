...
export async function renderCard(
  ...
  withDesignSpace(() => {
    ...
    if (data.playerType === 'star') {
      if (data.playsFor) drawWrappedParagraph(ctx, data.playsFor, 365, 840, { baseFont: opts.fonts.bodyFont, maxLines: 1, fitWidth: 400, fillStyle: opts.colors.text });
      if (data.specialRules) drawWrappedParagraph(ctx, data.specialRules, 365, 1250, { baseFont: opts.fonts.bodyFont, maxLines: 3, fitWidth: 400, fillStyle: opts.colors.text });
    } else {
      // Position
...
function drawWrappedParagraph(
  ...
  x: number,
  y: number,
  opts: { baseFont: string; maxLines: number; fitWidth: number; minFontPx?: number; expandToFourLinesThreshold?: number; fillStyle?: string }
) {
